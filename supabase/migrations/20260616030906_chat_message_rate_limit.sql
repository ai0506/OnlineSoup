-- Rate limit plain chat messages (message_mode = 'chat', cost 0):
-- at most 2 messages per second and 40 messages per minute, per seat.
-- Paid modes (ask/hint/reason) are not rate limited here; their cadence is
-- already governed by the points cost design.
...
begin
  ...
  if message_mode = 'chat' then
    ...
    where seat_id = sender_seat.id
      and message_mode = 'chat'
