...
revoke all on function public.record_login_context(text, text) from public, anon, authenticated;
grant execute on function public.record_login_context(text, text) to authenticated;
...
declare
  ...
  profile_device text;
begin
  if new.login_ip is null or new.login_device is null then
    select last_login_ip, last_login_device
    into profile_ip, profile_device
...
drop trigger if exists fill_points_transaction_login_context_trigger
  on public.points_transactions;

create trigger fill_points_transaction_login_context_trigger
  before insert on public.points_transactions
  for each row execute function public.fill_points_transaction_login_context();
...
begin
  ...
  return jsonb_build_object(
    'profile', jsonb_build_object(
      ...
      'last_login_ip',     profile_row.last_login_ip,
