-- Reasoning AI replies are stored as JSON that includes per-key-point coverage.
-- Keep player input validation in the send RPCs, but allow stored AI/system
-- messages to be long enough for structured results and puzzle reveals.
...
begin
  ...
  if is_success then
    if char_length(trim(ai_content)) not between 1 and 4000 then
