-- 推理判定为"推理正确"时，原先把汤底直接拼进一条 system 消息文本里，
-- 展示样式和普通系统消息一样朴素，汤底这种关键信息容易被忽略。这里把结算
-- 拆成两条消息：
-- a. 一条 system 消息，沿用"【动作】姓名 [座位] [身份] ..."的现有格式，
--    宣布是谁推理成功了哪道题（不包含汤底）。
-- b. 一条 message_type='ai' 的消息，和 AI 主持的其它回复同样的展示样式，
--    内容是完整汤底，让汤底在视觉上更突出。
