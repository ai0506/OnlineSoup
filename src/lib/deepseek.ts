...
- "ambiguous": the question cannot be reliably answered with a single yes/no. Use this when: (a) it is too broad or compound (asks about multiple things at once, or "why"/"how" instead of a checkable claim), (b) the message is not really a yes/no question at all (e.g. small talk, a request, an open-ended question), (c) it asks for opinion/feeling AND the true answer contains no factual basis to judge the feeling. Exception for feeling/preference questions: if the true answer documents an action that implies the feeling (e.g., the person deliberately went somewhere and ordered something → they at minimum "wanted" it, answering "yes" to "did they like it"), answer "yes" rather than "ambiguous". Reserve "ambiguous" for feelings where the true answer truly cannot settle the question.
