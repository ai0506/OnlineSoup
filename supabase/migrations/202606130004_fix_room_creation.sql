...
set nickname = profiles.display_name,
    occupied_at = coalesce(room_seats.occupied_at, now())
from public.rooms
join public.profiles on profiles.id = rooms.owner_id
where room_seats.room_id = rooms.id
  and room_seats.seat_number = 1
  and room_seats.nickname is null
