-- Add reply_to_id to room_messages so AI response messages can reference
-- the player's question that triggered them.
alter table public.room_messages
  add column if not exists reply_to_id bigint references public.room_messages(id) on delete set null;

create index if not exists room_messages_reply_to_id_idx
  on public.room_messages (reply_to_id)
  where reply_to_id is not null;

-- Update finish_room_ai_request to set reply_to_id = request_message_id
-- on the inserted AI message so admin audit can pair them.
