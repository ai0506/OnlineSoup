-- Fix open_puzzle 42702: rename parameter from puzzle_id to p_puzzle_id so it
-- no longer conflicts with the puzzle_progress.puzzle_id column reference.
-- The old overload (puzzle_id) must be dropped first to avoid PostgREST ambiguity.

drop function if exists public.open_puzzle(text, integer);
