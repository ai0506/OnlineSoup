...
> 本文件按当前工作区代码和迁移维护功能状态；最近核对：2026-09-19。`20260918025855_secure_room_ai_and_state_contracts.sql` 当前仍是未提交工作区改动，因此下列“已完成”仅表示本地实现与离线验证，不表示已经部署到生产。设计稿、线上数据库和部署配置与本文件冲突时，以实际代码、迁移和实测结果为准。
...
- [x] 询问 / 提示 / 尝试推理由 `send_room_ai_request` 原子扣除座位或个人积分；普通聊天入口只接受 `chat`，避免直接调用旧入口造成无 AI 回复的扣费
- [x] 座位方块角标实时显示临时积分
...
- [x] 页面重新可见 / 获焦 / 上线时触发同步
- [x] 订阅断开时自动补拉最新状态
- [x] 房间成员状态与当前题目经受保护 RPC 补拉；浏览器不再直接读取 `rooms`、`room_seats` 或事件表来恢复状态（迁移 `20260918025855`）
...
- [x] 同房间 AI 请求由 advisory lock 与 120 秒数据库租约严格串行；下一次请求前会结算已过期的 pending 请求（迁移 `20260918025855`）
- [x] 提示功能聚焦于未覆盖评分点
...
- [x] 管理端窄屏适配（`globals.css`）：tab 条与子 tab 条改内部横向滚动，标题/筛选表单竖排，账户卡片操作区单列，`.admin-tabs-shell > section` 加 `min-width: 0` 消除整页横向溢出；375px 下 7 个 tab 与新增题目/导入题库/缓存面板/AI 错误案例编辑各弹窗均已验证
...
- [x] 管理端按 tab 加载收紧（`src/app/admin/page.tsx`）：消息、待清理房间、活跃房间三个查询补上 tab 判断，未加载的 tab 不再显示误导性的 `0` 徽章；本地实测各 tab 服务端耗时下降约 2–3.5 倍
- [ ] **待做**：管理端积分流水按用户名筛选、账户搜索仍是「先取 300/1000 条再在内存过滤」，超出上限的记录搜不到；需把筛选下推到数据库并补分页和总数
...
- [x] 核心房间契约有 Vitest 与 pgTAP 测试骨架；GitHub Actions 在 PR 与 `main` 推送时运行 typecheck、lint、test、build
- [ ] **待做**：积分操作速率限制
- [ ] **待做**：Supabase 日志监控 / 告警
- [ ] **待做**：错误边界与用户友好错误页面
- [ ] **待做**：房间清理接入定时任务
...
4. **管理端数据正确性**：积分流水/账户搜索的筛选需下推到数据库并补分页，当前内存过滤会漏掉上限之外的记录。
