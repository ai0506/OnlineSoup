-- finish_room_ai_request never wrote the reasoning verdict back to
-- puzzle_progress.solved. The "✓ 已推理成功" badge and the puzzle list's
-- solved status were therefore always false even after a correct reasoning,
-- because nothing ever flipped puzzle_progress.solved to true.
--
-- The AI reasoning result is stored as JSON in room_messages.content with
-- shape {"kind":"reasoning_result","text":"推理正确"|"部分正确"|"推理不正确", ...}
-- (see formatReasonContent in src/lib/deepseek.ts). When message_mode is
-- 'reason' and the verdict text is "推理正确", mark the puzzle solved for
-- this room.
