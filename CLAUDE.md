...
- 动手前先给方案：说明要改什么、为什么这样改、会动到哪些文件、有什么风险。
- 方案可以包含技术细节（文件路径、函数名、SQL），但主线要让人看懂：先结论，再用简单语言说原因。
- 方案确认后一次做完：代码修改、数据库迁移、typecheck/lint 验证一起完成，不要做一半等追问。
- 错别字、格式、单行明显笔误等微小改动可以直接改，改完说明即可。
- 以下动作即使方案已确认，执行前仍须单独确认：向线上库 push 迁移、修改 RLS 或函数权限、删除或批量改写数据。
...
- 解决问题后，判断该问题是否值得沉淀为可复用的工程经验；例如涉及根因与表象不同、并发/权限/数据一致性/安全边界、线上或测试环境差异、容易重复出现的排障路径，或能形成明确预防规则的问题。
- 值得记录时，按 `lessons.md` 末尾的“后续记录格式”追加一条完整记录，包含问题、根因、处理方式、验证证据、经验、后续规则和对应的 `updates.md` 来源；不要把普通功能改动或单纯 changelog 重复写入。
- 只有在问题已经解决且具备足够事实与验证依据时才记录；不确定的推测应先明确验证，避免把猜测沉淀为项目规则。
